...
export function processDirectives(
  ...
  for (const el of elements) {
    ...
    if (
      skipHydrated &&
      (root instanceof Element ? isHydratedWithinRoot(el, root) : isHydratedElement(el))
    ) {
      skipRoots.push(el);
      continue;
    }
    ...
    if (
      el.tagName.includes("-") &&
      (!(root instanceof Element) || el !== root || treatRootAsBoundary)
    ) {
      skipRoots.push(el);
    }
  }
