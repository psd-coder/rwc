...
defineComponent("todo-app-spred", (ctx) => {
  ...
  // Computed
  const activeTodos = signal((get) => get(todos).filter((t) => !t.completed));

  const completedTodos = signal((get) => get(todos).filter((t) => t.completed));
  ...
  const canAdd = signal((get) => get(newTodo).trim().length > 0);
